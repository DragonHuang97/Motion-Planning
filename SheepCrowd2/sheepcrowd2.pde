...
int sphobs = 12; // number of spherical obstacles
int rectobs = 0; // number of rectangular obstacles
int agts = 6; //number of agents
...
//Description of PRM
int numofMS = 200; // number of milestones
...
void setup() {
  size(600, 600, P3D);
  surface.setTitle("Sheeps around woods");
...
void init() {
  ...
  sphobsPos[2] = new PVector(150, 45);
  sphobsPos[3] = new PVector(150, -35);
  sphobsPos[4] = new PVector(-75, 60);
  sphobsPos[5] = new PVector(-15, 90);
  sphobsPos[6] = new PVector(0, 0);
  sphobsPos[7] = new PVector(-150, 30);
  sphobsPos[8] = new PVector(100, 0);
  sphobsPos[9] = new PVector(-50, 90);
  sphobsPos[10] = new PVector(0, 30);
  sphobsPos[11] = new PVector(-100, 40);
...
void draw() {
  ...
  for (int i=0; i<rectobs; i++) {
    pushMatrix();
    translate(rectobsPos[i].x, 0, rectobsPos[i].y);
    popMatrix();
  }
  // draw Agent
  for (int i=0; i<agts; i++){
  ...
  for (int i=0; i<numofMS+agts; i++){
    stroke(255, 0, 0);
    strokeWeight(4);
    point(prm.mspos[i].x, -50, prm.mspos[i].y);
    strokeWeight(1);
